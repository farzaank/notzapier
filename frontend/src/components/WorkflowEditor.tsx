...
export default function WorkflowEditor({ id }: { id: number }) {
	...
	const saveParameters = (parameters: Record<string, string>) => {
		if (currentTaskIdx && selectedWorkflow && selectedWorkflow.tasks) {
			const task = selectedWorkflow.tasks[currentTaskIdx];
			const tasks = selectedWorkflow.tasks;
			tasks[currentTaskIdx] = {
				name: task.name,
				provider: task.provider,
				needsKey: task.needsKey,
				params: parameters,
			};
			updateTasksForWorkflow(tasks);
		}
		setCurrentTaskIdx(-1);
	};

	const saveWorkflow = () => {
		fetch(API_ENDPOINT + "/update-workflow", {
			method: "POST",
			headers: {
	...
	return (
		<div className="container mx-auto p-4 max-w-4xl">
			<Card>
				...
				<CardContent>
					{loading || selectedWorkflow === undefined ? (
						<LoadingSpinner></LoadingSpinner>
					) : (
						<div>
							<div className="mb-6">
								<h2 className="text-lg font-semibold mb-2">1. Trigger</h2>
								<Select onValueChange={addTrigger}>
									<SelectTrigger className="w-full">
										<SelectValue
											defaultValue={selectedWorkflow?.trigger}
											placeholder={selectedWorkflow?.trigger}
										/>
									</SelectTrigger>
									<SelectContent>
							...
							<div className="flex justify-end space-x-2">
								<Button
									...
									disabled={
										...
										selectedWorkflow.tasks.length < 2
									}
								>
...
function ParameterForm({ task, onSave }: ParameterFormProps) {
	...
	// TODO: read this from a config JSON file or DB instead of defining here
